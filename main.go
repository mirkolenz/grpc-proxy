...
var cli struct {
	...
	Rest struct {
		...
		ProtoDescriptorBin           string   `json:"proto_descriptor" type:"existingfile" help:"Supplies the binary content of the proto descriptor set for the gRPC services."`
		Services                     []string `json:"services" name:"services" help:" A list of strings that supplies the fully qualified service names (i.e. “package_name.service_name”) that the transcoder will translate. If the service name doesn't exist in proto_descriptor, Envoy will fail at startup. The proto_descriptor may contain more services than the service names specified here, but they won’t be translated. By default, the filter will pass through requests that do not map to any specified services. If the list of services is empty, filter is considered disabled. However, this behavior changes if reject_unknown_method is enabled."`
